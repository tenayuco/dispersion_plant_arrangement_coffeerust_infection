...
r=0.011  #growth rate of leaves
b1= 0.035 #primary infection rate
b2 = 0.035 #secondary infection rate
a=  0.65#new incorporation to the spore from infected leaf rate
mu= 0.2
x0 = [0,0, 0] #inital condition
tiempoReal = 301
sizeStep = 0.01 #euler integration step
...
liga = "patungDirectory"+ "/salida/matricesGenerales"
...
liga2 = "patungDirectory"+ "/salida/dfPromedios/"
dfPromedio.to_csv(liga2+"dfPromedio_%s.csv"%(args.code))



df_picosPlantas= df_calculadorPicosPlanta(matrizGeneral, dicODE, dicLattice, dicMigration, sim, marcadoresHechos, Dc)
liga3 = "patungDirectory"+"/salida/dfPicos/"
df_picosPlantas.to_csv(liga3+"df_PicosPlantas%s.csv" %(args.code))
