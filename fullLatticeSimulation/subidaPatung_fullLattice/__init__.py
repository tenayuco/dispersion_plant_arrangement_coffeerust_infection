...
def creadorDF_MaPromedio(matrizGen, dic_ODE, dic_Lattice, dic_Migration, sim, marcadores, Dc_):
    ...
    df_matrizPromedio["r"] = parameters[0]  
    df_matrizPromedio["b1"] = parameters[1]  
    df_matrizPromedio["b2"] = parameters[2]  
    df_matrizPromedio["a"] = parameters[4]  
    df_matrizPromedio["mu"] = parameters[5]
    df_matrizPromedio["sizeStep"] = sizeStep  
...
def df_calculadorPicosPlanta(matrizGen, dic_ODE, dic_Lattice, dic_Migration, sim, marcadores, Dc_):
    ...
    dataFrame_planta = pd.DataFrame(columns=["Sim", "Plant", "Imax", "TiempoImax", "Iini", "Ipro", "Patron", "Plot", "tasaMig", "g", "r", "b1", "b2","a","mu", "sizeStep"])
   ...
   # Ipro = Ires[1]
    ...
    for a in indPosibles:
       ...
       # print(x,y)
        ...
        tiempoImax = sizeStep*pasosImax
        dataFrame_planta = dataFrame_planta.append({"Sim":sim, "Dc": Dc, "Plant":[x,y], "Imax": Imax ,"TiempoImax":tiempoImax, "Ini":Iini , "Ipro":Ipro, "Patron":patron, "tasaMig":mig,"g":g, "r":r, "b1":b1, "b2":b2,"a":a,"mu":mu, "sizeStep": sizeStep},ignore_index = True)
